...
def load_data(dataset="scifact"):
    """
    Load and prepare the dataset for benchmarking.
    
    Args:
        dataset (str): Name of the dataset to load
        
    Returns:
        tuple: (corpus_df, queries_df, qrels)
    """
    ...
    return corpus_df, queries_df, qrels

def initialize_lotus():
...
if __name__ == "__main__":
    ...
    # Load data
    corpus_df, queries_df, qrels = load_data()
    ...
    util.save_runfile(os.path.join(results_dir, f"scifact.run.trec"), result)
    util.save_results(os.path.join(results_dir, f"scifact.json"), ndcg, _map, recall, precision, mrr)
